(() => {
  ...
  const deliveryExceptionsChartConfig = {
    ...
    dataLabels: {
      enabled: false,
      formatter: (val, opt) => `${Number.parseInt(val)}%`,
    },
    legend: {
      show: true,
      position: "bottom",
      ...
      columnDefs: [
        ...
        },
        {
          ...
          render: (data, type, full, meta) => {
            ...
            const $row_output = `<div class="d-flex justify-content-start align-items-center user-name"><div class="avatar-wrapper"><div class="avatar me-2"><span class="avatar-initial rounded-circle bg-label-secondary text-body"><i class="ti ti-truck"></i></span></div></div><div class="d-flex flex-column"><a class="text-body fw-medium" href="${baseUrl}app/logistics/fleet">VOL-${$location}</a></div></div>`;
            return $row_output;
          },
        },
        {
          ...
          targets: 3,
          render: (data, type, full, meta) => {
            const $start_city = full.start_city;
            const $start_country = full.start_country;
            const $row_output = `<div class="text-body">${$start_city}, ${$start_country}</div >`;
            return $row_output;
          },
        },
        {
          ...
          targets: 4,
          render: (data, type, full, meta) => {
            const $end_city = full.end_city;
            const $end_country = full.end_country;
            const $row_output = `<div class="text-body">${$end_city}, ${$end_country}</div >`;
            return $row_output;
          },
        },
        {
        ...
        },
        {
          ...
          render: (data, type, full, meta) => {
            ...
            const $progress_output = `<div class="d-flex align-items-center"><div div class="progress w-100" style="height: 8px;"><div class="progress-bar" role="progressbar" style="width:${$progress}%;" aria-valuenow="${$progress}" aria-valuemin="0" aria-valuemax="100"></div></div><div class="text-body ms-3">${$progress}%</div></div>`;
