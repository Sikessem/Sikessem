...
(() => {
  ...
  if (dt_academy_course.length) {
    const dt_course = dt_academy_course.DataTable({
      ...
      columnDefs: [
        ...
        },
        {
          ...
          render: (data, type, full, meta) => {
            ...
            const $row_output = `<div class="d-flex align-items-center"><span class="me-3">${logoObj[$logo]}</span><div><a class="text-body text-truncate fw-medium mb-2 text-wrap" href=" ${baseUrl}app/academy/course-details">${$course}</a><div class="d-flex align-items-center mt-1"><div class="avatar-wrapper me-2"><div class="avatar avatar-xs">${$output}</div></div><span class="text-nowrap">${$user}</span></div></div></div>`;
            return $row_output;
          },
        },
        {
          ...
          render: (data, type, full, meta) => {
            ...
            const Hs = Math.floor(duration.asHours());
            const minutes = Math.floor(duration.asMinutes()) - Hs * 60;
            const formattedTime = `${Hs}h ${minutes}m`;
        ...
        },
        {
          ...
          render: (data, type, full, meta) => {
            ...
            const $average_number = full.number;
        ...
        },
        {
          ...
          targets: 5,
          render: (data, type, full, meta) => {
            const $user_number = full.user_number;
