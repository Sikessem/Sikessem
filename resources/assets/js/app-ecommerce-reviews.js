...
$(() => {
  ...
  const dt_customer_review = $(".datatables-review");
  ...
  if (dt_customer_review.length) {
    const dt_review = dt_customer_review.DataTable({
      ...
      columnDefs: [
        ...
        },
        {
          ...
          render: (data, type, full, meta) => {
            ...
            const $company_name = full.company_name;
            const $id = full.id;
            ...
            if ($image) {
              ...
              const $product = full.product;
              let $initials = $product.match(/\b\w/g) || [];
              $initials = (
            ...
            const $row_output = `<div class="d-flex justify-content-start align-items-center customer-name"><div class="avatar-wrapper"><div class="avatar me-2 rounded-2 bg-label-secondary">${$output}</div></div><div class="d-flex flex-column"><span class="fw-medium text-nowrap">${$product}</span></a><small class="text-muted">${$company_name}</small></div></div>`;
            return $row_output;
          },
        },
        {
          ...
          render: (data, type, full, meta) => {
            const $name = full.reviewer;
            const $email = full.email;
            const $avatar = full.avatar;

            if ($avatar) {
              // For Avatar image
              const $output = `<img src="${assetsPath}img/avatars/${$avatar}" alt="Avatar" class="rounded-circle">`;
            } else {
              // For Avatar badge
              const stateNum = Math.floor(Math.random() * 6);
              const states = [
            ...
            // Creates full output for row
            const $row_output = `<div class="d-flex justify-content-start align-items-center customer-name"><div class="avatar-wrapper"><div class="avatar me-2">${$output}</div></div><div class="d-flex flex-column"><a href="${customerView}"><span class="fw-medium">${$name}</span></a><small class="text-muted text-nowrap">${$email}</small></div></div>`;
            return $row_output;
          },
        },
        {
          ...
          render: (data, type, full, meta) => {
            ...
            const $review = `<div>${$readOnlyRatings.prop(
              "outerHTML",
            )}<p class="h6 mb-1 text-truncate">${$firstCap}</p><small class="text-break pe-3">${$comment}</small></div>`;
